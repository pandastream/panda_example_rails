...
PandaUploader.bind=function(b,c){return function(){var f=b[c];if(f)return f.apply(b,arguments)}};PandaUploader.alert=function(b){return alert(b)};PandaUploader.parseJSON=function(b){return jQuery&&jQuery.parseJSON?jQuery.parseJSON(b):eval("("+b+")")};PandaUploader.BaseWidget=function(){};
...
a.uploadStart=function(){this.swfupload.data("__swfu").setPostParams(this.getSignedParams());this.triggerEvent("onloadstart")};a.uploadProgress=function(b,c,f,k){b.loaded=f;b.total=k;this.triggerEvent("onprogress",[b])};a.uploadSuccess=function(b,c,f){this.setValue(PandaUploader.parseJSON(f).id);b={target:{status:"200",responseText:f}};this.triggerEvent("onreadystatechange",[b]);this.triggerEvent("onsuccess",[b])};a.getFile=function(){return this.file};a.start=function(){return this.swfupload.swfupload("startUpload")};
...
false);$(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"));this.query.after('<input type="file" accept="application/mp4" />');jQuery(this.getField()).change(this.boundHandler("onchange"));this.triggerEvent("onwidgetload")};
a.start=function(){var b=this.getFile(),c=this.getSignedParams();c='{"access_key":"'+c.access_key+'", "cloud_id":"'+c.cloud_id+'", "timestamp":"'+c.timestamp+'", "signature":"'+c.signature+'"}';this.xhr.open("POST",this.options.api_url+"/videos.json",true);this.xhr.setRequestHeader("Cache-Control","no-cache");this.xhr.setRequestHeader("Content-Type","application/octet-stream");this.xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");this.xhr.setRequestHeader("X-File-Name",b.fileName);this.xhr.setRequestHeader("X-Query-Params",
c);this.xhr.send(b)};a.abort=function(){this.xhr.abort()};a.getFile=function(){return jQuery(this.getField()).get(0).files[0]};a.setValue=function(b){return this.query.val(b)};a.disable=function(){jQuery(this.getField()).attr("disabled",true)};a.enable=function(){jQuery(this.getField()).removeAttr("disabled")};a.getField=function(){return this.query.next().get(0)};
a.onreadystatechange=function(b){this.triggerEvent("onreadystatechange",arguments);var c=null;try{c=b.target.status}catch(f){$(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"));return}if(c=="200"&&b.target.responseText){this.setValue(PandaUploader.parseJSON(b.target.responseText).id);this.triggerEvent("onsuccess",[b])}else $(this.xhr).one("readystatechange",PandaUploader.bind(this,"onreadystatechange"))};PandaUploader.BaseStrategy=function(){};
